...
Симуляция игрока (6 тапов/с, жадные покупки, без питомцев/бустов) — `lune run tests/balance.luau`:

| Время | Что происходит |
|---|---|
| 0:07 | первый апгрейд TAP POWER x2 |
| ~1:00 | 10 m → открыт Normal World |
| **~2:55** | **первый Rebirth (100 m) → x2** |
| ~6:40 | 1K m → Cloud World |
| ~10:40 | 10K m → Sky World |
| ~28 мин | 1M m → Space |
| ~2.5 ч | 1B m → Galaxy |
| ~5 ч | 1T m, Rebirth ~каждые 15–20 мин |
| ~10 ч | 1Qa m, R32 (x135) |
...
| Система | Коротко |
|---|---|
| **Tap** | +TapPower cm и монеты за тап; монеты = (рост за тап)^0.8 × множители монет × бонус за текущий рост |
...
| **Зоны** | Tiny Land, Normal World (10 m), Giant City (100 m), Cloud World (1K), Sky World (10K), Space (1M), Galaxy (1B); множители x1…x10, свой вид и освещение; открываются по лучшему росту навсегда; ворота + телепорт 🌍 WORLDS |
...
| **Питомцы / яйца** | Basic 100, Big 10K, Giant 1M, Galaxy 1B монет; Common…Secret (Secret 1 : 50 000); x1.1…x500; складываются аддитивно (3 × x5 = x13), чтобы не затмевать рост; Hatch 1 / 3, авто-экипировка лучших |
...
| **События сервера** | MEGA GROWTH x10 (60 с), GOLDEN MINUTE x5 монет, GIANT MODE (все огромные), TINY MODE (все крошечные, рост x100, 20 с), DOUBLE TAP; каждые 4–7 мин; участникам +гемы |
...
| **Quests** | 3 в день: 1,000 тапов, вырасти на 1,000 m, 1 Rebirth |
...
```bash
lune run tests/run.luau          # 47 юнит-тестов: числа, форматирование, форма тела, формулы, сохранения, АНТИ-ЧИТ, награды, баланс
rojo sourcemap default.project.json -o sourcemap.json
lune run tests/e2e.luau          # 125 end-to-end проверок: НАСТОЯЩИЙ серверный и клиентский код в мини-движке
