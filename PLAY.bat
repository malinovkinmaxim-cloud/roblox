@echo off
rem ============================================================
rem  THE DOPPELGANGER OBBY - one-click launch (Windows)
rem  1) gets Rojo (downloads it once into tools\ if needed)
rem  2) builds DoppelgangerObby.rbxlx from src\
...
if defined ROJO (
    ...
    "%ROJO%" build default.project.json -o DoppelgangerObby.rbxlx
...
if not exist "DoppelgangerObby.rbxlx" (
    echo ERROR: DoppelgangerObby.rbxlx not found.
    pause
    exit /b 1
)
...
if defined STUDIO (
    start "" "%STUDIO%" "%CD%\DoppelgangerObby.rbxlx"
) else (
    start "" "DoppelgangerObby.rbxlx"
)
